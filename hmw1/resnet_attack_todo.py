...
#rewrite ImageClassfication without normalization
...
#because when you preprocess with normalization, the range will not be with [0,1]
#Therefore, I mannually complete
class ResNetNormalization(nn.Module):
    def __init__(self, resnet, mean, std):
        super(ResNetNormalization, self).__init__()
        self.resnet = resnet
        self.mean = mean
        self.std = std
...
class ResnetPGDAttacker:
    ...
    def pgd_attack(self, image, label, eps=None, alpha=None, steps=None):
        ...
        pass  # TODO
        adv_images += torch.zeros_like(adv_images).uniform_(-eps, eps)
        for _ in range(steps):
            ...
            adv_images = images.clone().detach() + delta
    ...
    def pgd_batch_attack(self, eps, alpha, steps, batch_num):
        ...
        for i, inputs in enumerate(tqdm(self.dataloader, total=batch_num)):
            ...
            adv_images_lst.append(adv_images.cpu())
